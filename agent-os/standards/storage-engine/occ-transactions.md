...
4. **Commit** — Validation: check no page in `ReadSet` was modified by a concurrent committed tx since the recorded version. If ok, flush `WriteSet` to the pager via WAL (`commitWithWAL`), mark `TxCommitted`.
