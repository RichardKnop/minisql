...
func TestNewDatabase_MultipleTablesWithIndexes(t *testing.T) {
	...
	assert.NotNil(t, aDatabase.tables[testTableName3].UniqueIndexes[indexName])
	...
	assert.Equal(t, PageIndex(5), aDatabase.tables[testTableName3].UniqueIndexes[indexName].Index.GetRootPageIdx())
