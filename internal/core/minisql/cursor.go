...
func (c *Cursor) LeafNodeInsert(ctx context.Context, key uint64, aRow *Row) error {
	aPage, err := c.Table.pager.GetPage(ctx, c.PageIdx, c.Table.RowSize)
	if err != nil {
...
func (c *Cursor) LeafNodeSplitInsert(ctx context.Context, key uint64, aRow *Row) error {
	...
	aSplitPage, err := aPager.GetPage(ctx, c.PageIdx, c.Table.RowSize)
	if err != nil {
		return fmt.Errorf("get page: %w", err)
	}
	...
	aNewPage, err := aPager.GetFreePage(ctx, c.Table.RowSize)
	if err != nil {
	...
	aParentPage, err := aPager.GetPage(ctx, parentPageIdx, c.Table.RowSize)
	if err != nil {
